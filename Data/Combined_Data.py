        ...
        with multiprocessing.Pool(processes=multiprocessing.cpu_count()//8) as pool:
            for exercise in self.args.exercises:
                ...
                if self.args.force_regression:
                    ...
                    forces_async = pool.map_async(
                        self.utils.getForces, 
                        list(
                            zip(
                                [(i+1) for i in range(self.utils.num_subjects)], 
                                exercise*np.ones(self.utils.num_subjects).astype(int)), 
                                args*np.ones(self.utils.num_subjects).astype(int)
                                ))

...
class IRM_Trainer(Model_Trainer):
    """
    ...
    def pretrain_and_finetune(self, testing_metrics):
        ...
        ml_utils.evaluate_model_on_test_set(self.model, self.test_loader, self.device, self.num_gestures, F.cross_entropy, self.args, testing_metrics)
        ...
        for epoch in tqdm(range(ft_epochs), desc="Finetuning Epoch"):
            ...
            with tqdm(finetune_loader, desc=f"Finetuning Epoch {epoch+1}/{ft_epochs}", leave=False) as t:
                for X_batch, Y_batch in t:
                    ...
                    loss_ce = F.cross_entropy(output, Y_batch)
                    ...
                    loss_penalty += self.criterion(output_for_loso, labels_for_loso) / (self.utils.num_subjects - 1)
        ...
        device, self.num_gestures, F.cross_entropy, self.args, testing_metrics)
    ...
    def train_and_validate(self, training_metrics, validation_metrics):
        ...
        for epoch in tqdm(range(self.num_epochs), desc="Epoch"):
            ...
            with tqdm(self.train_loader, desc=f"Epoch {epoch+1}/{self.num_epochs}", leave=False) as t:
                ...
                for X_batch, Y_batch in t: 
                    ...
                    for output_per_domain, labels_per_domain in zip(output_per_domains, labels_per_domains):
                        loss_penalty += self.criterion(output_per_domain, labels_per_domain) / (self.utils.num_subjects - 1)
            ...
            if len(true_labels) == len(test_predictions):
                ...
                print("Error: Length of true labels and test predictions do not match.")
                conf_matrix = None
            print("Confusion Matrix:")
            print(conf_matrix)

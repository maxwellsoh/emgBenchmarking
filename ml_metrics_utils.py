...
def calculate_tpr_at_fpr(y_true, y_scores, fpr_target):
    ...
    # Interpolate the TPR at the target FPR if it does not exist exactly in the fpr array
    if fpr_target in fpr:
        return tpr[np.where(fpr == fpr_target)[0][0]]
    else:
        # Use interpolation to estimate the TPR at the target FPR
...
def evaluate_model_tpr_at_fpr(model, loader, device, num_classes, fpr_targets=[0.01, 0.1, 0.5]):
    ...
    for class_index in range(num_classes):
        y_true_class = (true_labels == class_index).astype(int)
    ...
    with torch.no_grad():
        for X_batch, Y_batch in loader:
            ...
            for thresh in thresholds:
                ...
                correct_counts[thresh] += (preds[above_thresh] == Y_batch[above_thresh]).sum().item()

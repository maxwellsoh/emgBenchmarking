...
#         data = torch.from_numpy(np.loadtxt(os.path.join(root, file), dtype=np.float16, skiprows=1)[:, 1:]).unfold(dimension=0, size=wLenTimesteps, step=stepLen)
        
#         emg.append(data[:, :-1][balance(data[:, -1])])
#         restimulus.append(data[:, -1][balance(data[:, -1])])

#     print(torch.cat(emg, dim=0).shape)
#     print(contract(torch.cat(restimulus, dim=0)).shape)
